...
2026-08-30：根据产品讨论整理绣绘 2.0 PRD，见 `docs/PRD_XIUhUI_2.0.md`。文档覆盖图片转刺绣、制版向导、针法编辑、绣框、仿真、质量检查、界面模式、版本路线、稳定性、授权和 GPL 合规要求。
...
- 下一步：正式发放时按芯片发送对应 DMG，并按订单记录设备码与授权信息
...
- 全新未激活 Apple Silicon 客户机已完成实际安装验证：清除隔离属性后可正常打开并进入激活流程。问题原因是微信传输给 app 内部文件递归写入 `com.apple.quarantine`，不是包损坏、签名失败或激活逻辑错误。
- Intel 与 Apple Silicon 均已由用户确认绘制不崩、不闪；两种正式 DMG 均通过 `hdiutil verify` 与 `xcrun stapler validate`。
- 2026-08-30 激活复测：删除钥匙串 `license-token` 后，现有 `/Applications` 副本弹出“无法验证开发者”。删除授权数据不会改变代码签名；检查下载目录中的最终 Apple Silicon DMG 后，DMG 的 stapler ticket、内部 app 的 Developer ID 签名和 Gatekeeper assessment 均有效。初步判断打开的是最终公证包生成前安装的旧副本，或系统保留了旧副本的隔离/评估状态。必须删除旧 app，再从最终正式客户版 DMG 重新拖入后继续首次激活验收。
...
- [x] 改画布刷新路径，验证“一画就整幅闪”能否减轻
- [x] 出 cairo 防崩测试包，复测导入图、持续绘制、本机和外接屏
- [ ] 把 `v0.2.x` 正式写入 `CHANGES.md` 和发布说明
- [x] 明确当前客户包标准：不崩、不闪，并完成双架构用户验证
